...
class Actor(nn.Module):
    ...
    def get_action_test(self, state):  # for testing
        action = self.net(state).tanh()
        return action
...
class ActorSAC(nn.Module):
    def __init__(self, mid_dim, state_dim, action_dim):
        super().__init__()
        self.net_state = nn.Sequential(
            nn.Linear(11, mid_dim),
            nn.ReLU(),
            nn.Linear(mid_dim, mid_dim),
            nn.ReLU(),
        )
        
        self.net_dnn = nn.Sequential(
            nn.Linear(70, 1),
            nn.Tanh(),
        )
        
        # self.net_sigmoid= nn.Sequential(
        #     nn.Sigmoid()
        # )
    ...
    def forward(self, state):
        ...
        tmp = self.net_dnn(state[:,10:80])
        tmp = tmp / 10
        tmp = (tmp + state[:,80:]).sigmoid()
        result = self.net_state(torch.cat((tmp, state[:,:10]), dim=1))
                                
        return self.net_a_avg(result).tanh()  # action

    def get_action(self, state):
        t_tmp = self.net_dnn(state[:,10:80])
        t_tmp = t_tmp / 10
        t_tmp = (t_tmp + state[:,80:]).sigmoid()
        result = self.net_state(torch.cat((t_tmp, state[:,:10]), dim=1))
        a_avg = self.net_a_avg(result)  # NOTICE! it is a_avg without .tanh()
        a_std = self.net_a_std(result).clamp(-20, 2).exp()
    ...
    def get_action_test(self, state):
        ...
        result = self.net_state(torch.cat((t_tmp, state[:,:10]), dim=1))
        a_avg = self.net_a_avg(result)  # NOTICE! it is a_avg without .tanh()
        return a_avg.tanh()  # re-parameterize
    ...
    def get_action_logprob(self, state):
        ...
        t_tmp = (t_tmp + state[:,80:]).sigmoid()
        result = self.net_state(torch.cat((t_tmp, state[:,:10]), dim=1))
        a_avg = self.net_a_avg(result)  # NOTICE! it needs a_avg.tanh()
        a_std_log = self.net_a_std(result).clamp(-20, 2)
        a_std = a_std_log.exp()
