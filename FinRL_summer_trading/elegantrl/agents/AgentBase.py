...
class AgentBase:
    ...
    def explore_one_env(self, env, target_step) -> list:
        ...
        self.is_train = getattr(env,"is_train",True)
...
#         print('self.is_train',self.is_train)
        while step_i < target_step or not done:
            ...
            if self.is_train:
                ...
                ten_a = (self.act.get_action_test(ten_s.to(self.device)).detach().cpu())  # different
                
                
            next_s, reward, done, _ = env.step(ten_a[0].numpy())  # different
#             print('in agent base, reward shape',reward.shape)
#             ten_a = ten_a.reshape(-1)
    ...
    def convert_trajectory(self, buf_items, last_done):  # [ElegantRL.2022.01.01]
        ...
        # buf_items = [buf_item, ...]
        if buf_items[1].ndim == 3:
            buf_items[1] = buf_items[1].squeeze(2)
        return buf_items
